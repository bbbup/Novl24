...
cron "20 0,19 * * *" script-path=jd_speed_redpocke.js,tag=京东极速版红包
...
function reward_query() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_query", {
      "inviter": ["oJsReO8VOzBZ8oPN3gMsETD4GObzHcFB1k0L_Jf13bY"][Math.floor((Math.random() * 1))],
      linkId
    }), async (err, resp, data) => {
      try {
        if (err) {
...
async function redPacket() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_receive", { "inviter": ["oJsReO8VOzBZ8oPN3gMsETD4GObzHcFB1k0L_Jf13bY"][Math.floor((Math.random() * 1))], linkId }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`)
