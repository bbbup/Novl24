...
!(async () => {
  ...
  for (let index = 0; index < 8; index++) {
    ...
    for (let k = 0; k < orderList.length; k++) {
      ...
      if (k === 0) {
        console.log('====================================');
        console.log(`🙆🏻‍♂️账号：${userInfo.nickname}`);
        allMessage += `===================================\n`;
        allMessage += `🙆🏻‍♂️账号：${userInfo.nickname}\n`;
      }
      // 忽略取消订单以及非实物订单
      if (
        stateName !== '已完成' &&
...
function getUserInfo() {
  return new Promise((resolve) => {
    ...
    opts.headers.Referer = `https://wqs.jd.com/my/jingdou/my.shtml?sceneval=2`;

    $.post(opts, (err, resp, data) => {
      let userInfo;

      try {
        userInfo = JSON.parse(data).base;
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve(userInfo);
...
function showMsg(userInfo, wuLiuDetail, orderId) {
  return new Promise((resolve) => {
    ...
    }\n📘订单编号：${orderId}\n📕包含商品：${orderWareList[0].itemName.slice(0,20).replace("&","")}\n`;
